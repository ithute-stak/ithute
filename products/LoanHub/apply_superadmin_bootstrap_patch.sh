...
if [[ -d "${PROJECT_ROOT}/apps/backend" ]]; then
    ...
    echo "Could not find apps/backend/main.py or main.py."
    exit 1
fi

MAIN_FILE="${BACKEND_ROOT}/main.py"
SERVICE_FILE="${BACKEND_ROOT}/services/bootstrap_superadmin.py"

if [[ ! -f "${MAIN_FILE}" ]]; then
    echo "Backend main.py was not found at ${MAIN_FILE}."
    exit 1
fi

mkdir -p "${BACKEND_ROOT}/services"

cat >"${SERVICE_FILE}" <<'PYTHON'
...
marker = "# LoanHub initial super-admin bootstrap"
...
if marker not in source:
    main_file.write_text(
        source.rstrip() + bootstrap_block + "\n",
        encoding="utf-8",
    )
    print(f"Updated {main_file}")
else:
    print(f"Bootstrap already installed in {main_file}")
PYTHON
...
echo
echo "Super-admin bootstrap patch applied successfully."
echo "Changed files:"
echo "  ${SERVICE_FILE}"
echo "  ${MAIN_FILE}"
echo
echo "Configure BOOTSTRAP_SUPERADMIN_PASSWORD before restarting."