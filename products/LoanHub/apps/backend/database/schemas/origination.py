...
class DebtObligationInput(BaseModel):
    ...
    installment_amount: Decimal = Field(default=0, ge=0)
    installment_frequency: Literal["weekly", "fortnightly", "monthly", "quarterly", "custom"] = "monthly"
    ...
    status: Literal["active", "settled", "defaulted", "restructured", "written_off", "unknown"] = "active"
    source: str = Field(default="declared", max_length=60)
    is_verified: bool = False
    notes: str | None = Field(default=None, max_length=4000)

    @model_validator(mode="after")
    def validate_debt_schedule(self):
        ...
        if self.status in {"active", "defaulted", "restructured", "unknown"} and self.current_balance > 0:
            if self.installment_amount <= 0 and self.monthly_installment <= 0:
...
class FinancialProfileUpdate(BaseModel):
    ...
    bank_accounts: list[BankAccountInput] = Field(default_factory=list, max_length=10)
    bank_account: BankAccountInput | None = Field(
        default=None,
        description="Deprecated single-account compatibility field.",
    )

    @model_validator(mode="after")
    def normalize_bank_accounts(self):
        if self.bank_account is not None and not self.bank_accounts:
            self.bank_accounts = [self.bank_account]
        if sum(1 for account in self.bank_accounts if account.salary_account) > 1:
            raise ValueError("Only one bank account may be marked as the salary account")
        ids = [account.id for account in self.bank_accounts if account.id is not None]
        if len(ids) != len(set(ids)):
            raise ValueError("The same bank account cannot be submitted twice")
        return self
...
class ContractRegenerateRequest(BaseModel):
    ...
    template_style: ContractTemplateStyle | None = None
