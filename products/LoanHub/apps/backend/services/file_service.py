...
from core.access_control import COMPANY_MANAGEMENT_ROLES, LENDING_ROLES, TenantContext
...
from services.malware_scan_service import (
...
def _scan_content(content: bytes) -> str:
    try:
        result = scan_upload(content)
    except MalwareDetectedError as error:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail='The uploaded file failed the malware safety check',
        ) from error
    except MalwareScannerUnavailableError as error:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail='File uploads are temporarily unavailable while the malware scanner recovers',
        ) from error
    return 'clean' if result == 'clean' else 'validated'


def _write_payload(key: str, payload: bytes):
    storage = get_storage()
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail='Managed file storage is temporarily unavailable',
        ) from error
    return storage
...
async def save_upload(
    ...
    *,
    ...
    scan_status = _scan_content(plaintext)
    ...
    storage = _write_payload(str(relative_key), payload)
...
def store_bytes(
    ...
    *,
    ...
    scan_status = _scan_content(content)
    stored_name = f'{uuid.uuid4().hex}.bin' if settings.FILE_ENCRYPTION_ENABLED else (
        f"{uuid.uuid4().hex}{('.' + extension) if extension else ''}"
    )
...
def physical_path(file: ManagedFile) -> Path:
    """Return the local legacy path; remote objects do not have a physical path."""
    if (file.storage_provider or 'local').lower() != 'local':
        raise HTTPException(status_code=400, detail='Remote managed files do not have a local path')
    root = storage_root()
...
def read_file_bytes(file: ManagedFile) -> bytes:
    try:
        content = get_storage(file.storage_provider or 'local').get(file.storage_key)
    except FileNotFoundError as error:
        raise HTTPException(status_code=410, detail='Stored file is unavailable') from error
    except StorageUnavailableError as error:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail='Managed file storage is temporarily unavailable',
...
def _can_access_borrower_evidence(
    db: Session,
    context: TenantContext,
    file: ManagedFile,
) -> bool:
    if (
        ...
        or not file.linked_entity_id
    ):
        return False
    try:
        borrower_id = uuid.UUID(file.linked_entity_id)
    ...
    if not borrower:
        return False
    if borrower.user_id == context.user.id:
        return True
    if (
        not context.company_id
        or not context.staff
        or context.staff.role not in LENDING_ROLES
    ):
        return False

    if file.company_id == context.company_id:
        return True
    if not (
        borrower.consent_to_share_profile
        and borrower.consent_to_share_documents
    ):
        return False
    ...
    concerned = bool(
        ...
        .first()
        or db.query(DirectLoanApplication.id)
        .filter(
            DirectLoanApplication.company_id == context.company_id,
            DirectLoanApplication.borrower_id == borrower.id,
        )
        .first()
        or db.query(ClientCompanyLoan.id)
        .filter(
...
def can_access_file(db: Session, context: TenantContext, file: ManagedFile) -> bool:
    ...
    if file.owner_user_id == context.user.id:
        return True
    if _can_access_borrower_evidence(db, context, file):
        return True
