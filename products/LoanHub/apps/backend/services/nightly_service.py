...
from database.models.professional_lending import CreditBlacklist
...
def run_midnight_reconciliation(db: Session, local_date: date | None = None) -> dict[str, int]:
    ...
    stale = db.query(ClientCompanyLoan).filter(ClientCompanyLoan.status == LoanStatus.DEFAULTED, ClientCompanyLoan.updated_at < cutoff).all()
    for loan in stale:
        existing = db.query(CreditBlacklist).filter(CreditBlacklist.borrower_id == loan.borrower_id).first()
        if existing:
            existing.is_active = True
            existing.reason = 'Loan has remained unpaid/defaulted for at least 365 days'
        else:
            db.add(CreditBlacklist(borrower_id=loan.borrower_id, reason='Loan has remained unpaid/defaulted for at least 365 days', source='automatic_365_day_default', is_active=True, blacklisted_at=datetime.now(ZoneInfo(settings.APP_TIMEZONE))))
    db.commit()
    logger.info(
