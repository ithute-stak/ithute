...
from services.installment_service import extend_installment_due_date
...
def test_due_date_extension_changes_date_only_and_writes_audit():
    loan = _loan()
    db = FakeSession()
    first = loan.installments[0]

    extend_installment_due_date(
        db,
        loan=loan,
...
def test_due_date_extension_cannot_shorten_date():
    loan = _loan()
...
def test_due_date_extension_preserves_schedule_order():
    loan = _loan()
    with pytest.raises(HTTPException) as caught:
        extend_installment_due_date(
            FakeSession(),
