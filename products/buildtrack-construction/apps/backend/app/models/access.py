...
class UTCDateTime(TypeDecorator[datetime]):
    """Return timezone-aware UTC values on every supported database backend.

    PostgreSQL preserves timezone metadata for ``TIMESTAMP WITH TIME ZONE``, while
    SQLite drops it. BuildTrack's tests and local tooling use SQLite in places,
    so normalising at the ORM boundary prevents security expiry/lockout checks
    from ever comparing offset-naive and offset-aware values.
    """
...
class User(Base):
    ...
    password_changed_at: Mapped[datetime] = mapped_column(UTCDateTime(), nullable=False, default=utcnow)
...
class SecurityEvent(Base):
    ...
    occurred_at: Mapped[datetime] = mapped_column(UTCDateTime(), nullable=False, default=utcnow, index=True)
