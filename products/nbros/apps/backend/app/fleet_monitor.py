...
def recipient_subs(db, branch_id: uuid.UUID) -> list[str]:
    admin_ids = set(
        db.scalars(
            select(Profile.auth_user_id).where(Profile.role.in_(["admin", "fleet_admin"]))
        )
    )
    branch_ids = set(
        db.scalars(
            select(Profile.auth_user_id)
            .join(ProfileBranchAccess, ProfileBranchAccess.profile_id == Profile.id)
...
def monitor_once(redis_client: Redis) -> dict[str, int | str]:
    ...
    try:
        with SessionLocal() as db:
            branch_ids = select(BranchModule.branch_id).where(
            ...
            for vehicle in vehicles:
                ...
                changed_rows = sync_vehicle_alerts(
                    db,
        ...
        redis_client.set("nbros:fleet-monitor:last-run", json.dumps(status), ex=86400)
        return status
    finally:
...
def main() -> None:
    ...
    while True:
        ...
        try:
            result = monitor_once(redis_client)
            log.info("Fleet monitor result: %s", result)
        except Exception:
