...
def branch(db: Session) -> Branch:
    ...
    db.add(row)
    db.flush()
    db.add(BranchModule(branch_id=row.id, module_key="fleet", is_enabled=True))
    db.add(
        FleetSetting(
            branch_id=row.id,
...
def healthy_vehicle(db: Session, branch: Branch) -> Vehicle:
    vehicle = Vehicle(
        branch_id=branch.id,
        registration_plate="A100",
        make="Toyota",
        model="Hilux",
        vehicle_type="Light vehicle",
        current_mileage=10_000,
        mechanical_condition="operational",
    )
    db.add(vehicle)
    db.flush()
    db.add(
    ...
    db.add(
        Inspection(
            ...
            status="passed",
            inspector="Fleet Test",
            next_inspection_date=date(2027, 1, 1),
        )
    )
    db.commit()
    return vehicle
